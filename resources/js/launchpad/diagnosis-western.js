$(document).ready(function(){
    ...
    var dxTypes = $(".item, .itemFU").filter(function(){
        return $(this).children(".question").find(".q").text().toLowerCase().includes("type of diagnosis");
    }).first().children(".answer").find("li");
    
    var Dxs = [], DxStr = "";
    dxTypes.each(function(d,Dx){Dxs.push($(Dx).text());})
    Dxs.forEach(function(Dx,d){
        DxStr += "<label><input type='checkbox' class='tableFilter' data-filter='type:"+Dx+"'>"+Dx+"</label>";
    })
    $("#DxType").html(DxStr);
