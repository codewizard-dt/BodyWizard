...
try {
	...
	// $list = $table_access ? json_encode($class::list()) : '';
} catch (\Exception $e) {
	reportError($e);
	$index = false; $table_str = '';
}
// $instance = isset($uid) ? $class::find($uid) : null;
?>

<div id="Select{{$model}}Table" class="Table central full" data-is_index='{{$index ? "true" : "false"}}' {!!$table_str!!}>
