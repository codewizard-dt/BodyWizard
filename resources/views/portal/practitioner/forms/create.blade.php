...
$inputs = [];
set($inputs, 'number.min', new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'initial'], [-99999, 99999, 0, 1, '', 'Minimum:', 0]), 'number.max', new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'initial'], [-99999, 99999, 0, 1, '', 'Maximum:', 10]), 'number.start', new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'initial'], [-99999, 99999, 0, 1, '', 'Initial:', 5]), 'number.step', new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'initial'], [0, 99999, 1, 0.1, 'units', 'Increment:', 1]), 'number.units', new_input('text', ['preLabel', 'placeholder'], ['Units:', 'eg days, weeks, times/day, meals, etc']), 'number.units.settings.required', 'false', 'number.preLabel', new_input('text', ['preLabel', 'placeholder'], ['Text label:', 'optional']), 'number.preLabel.settings.required', 'false');
set($inputs, 'text.placeholder', new_input('text', ['placeholder', 'preLabel'], ['(optional) disappears when you type', 'Placeholder text:']), 'text.placeholder.settings.required', 'false');
set($inputs, 'textbox.placeholder', new_input('textbox', ['placeholder', 'preLabel'], ['(optional) disappears when you type', 'Placeholder text:']), 'textbox.placeholder.settings.required', 'false');
set(
    $inputs,
    'date.limit',
    new_input('list', ['list', 'listLimit', 'preLabel'], [['one', 'two', 'three', 'five', 'ten', 'no limit'], 1, 'Limit selection to']),
    'date.min.num',
    new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'labelClass'], [0, 100, 0, 1, '', 'FROM', 'pink']),
    'date.min.num.settings.required',
    'false',
    'date.min.type',
    new_input('dropdown', ['list'], [['days', 'weeks', 'months', 'years']]),
    'date.min.type.settings.required',
    'false',
    'date.min.dir',
    new_input('dropdown', ['list', 'postLabel'], [['before', 'after'], 'current date']),
    'date.min.dir.settings.required',
    'false',
    'date.max.num',
    new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel', 'labelClass'], [0, 100, 0, 1, '', 'UNTIL', 'pink']),
    'date.max.num.settings.required',
    'false',
    'date.max.type',
    new_input('dropdown', ['list'], [['days', 'weeks', 'months', 'years']]),
    'date.max.type.settings.required',
    'false',
    'date.max.dir',
    new_input('dropdown', ['list', 'postLabel'], [['before', 'after'], 'current date']),
    'date.max.dir.settings.required',
    'false',
);
set($inputs, 'bodyclick.size', new_input('list', ['listLimit', 'list', 'preLabel'], ['1', ['small', 'medium', 'large', 'x-large'], 'Image size:']), 'bodyclick.bodyclickSample', new_input('bodyclick', ['size'], ['small']));
set($inputs, 'bodyclick.bodyclickSample.settings.required', 'false');
set($inputs, 'scale.dispVal', new_input('dropdown', ['list', 'preLabel'], [['yes', 'no'], 'Show current value']), 'scale.dispLabel', new_input('dropdown', ['list', 'preLabel'], [['yes', 'no'], 'Show min/max labels']), 'scale.leftLabel', new_input('text', ['placeholder', 'preLabel'], ['appears to left of scale', 'Left text label']), 'scale.rightLabel', new_input('text', ['placeholder', 'preLabel'], ['appears to right of scale', 'Right text label']), 'scale.min', new_input('number', ['min', 'max', 'start', 'step', 'preLabel'], [-1000, 1000, 0, 1, 'Minimum value:']), 'scale.max', new_input('number', ['min', 'max', 'start', 'step', 'preLabel'], [-1000, 1000, 100, 1, 'Maximum value:']), 'scale.start', new_input('number', ['min', 'max', 'start', 'step', 'preLabel'], [-1000, 1000, 50, 1, 'Initial value:']));
set(
    $inputs,
    ...
    'time.min',
    new_input('time', ['preLabel'], ['Earliest allowed:']),
    'time.min.settings.required',
    'false',
    'time.max',
    new_input('time', ['preLabel'], ['Latest allowed:']),
    'time.max.settings.required',
    'false',
    'time.step',
    new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel'], [1, 1000, 15, 1, 'minutes', 'Time increment:']),
    'time.step.settings.required',
    'false',
);
set($inputs, 'time2.min2', new_input('time', ['preLabel'], ['Earliest allowed:']), 'time2.min2.settings.required', 'false', 'time2.max2', new_input('time', ['preLabel'], ['Latest allowed:']), 'time2.max2.settings.required', 'false', 'time2.step2', new_input('number', ['min', 'max', 'start', 'step', 'units', 'preLabel'], [1, 1000, 15, 1, 'minutes', 'Time increment:']), 'time2.step2.settings.required', 'false');
set($inputs, 'SectionName', new_input('text', ['placeholder', 'on_enter_action', 'html_tag', 'input_css'], ['Section Name', 'Section.create', 'h2', ['textAlign' => 'center']]), 'SectionName.settings.placeholder_shift', 'false');
$categories = collect(['Patient', 'Practitioner', 'Staff Member', 'Service', 'Form', 'Complaint', 'Complaint Category'])
    ->map(function ($model) {
        return ['text' => $model, 'value' => toKeyString($model), 'data' => ['plural' => plural($model)]];
    })
    ->toArray();

$AutofillSettingsPrompt_buttons = [['text' => 'continue', 'class_list' => 'pink', 'action' => 'Item.AutofillSettingsConfirm'], ['text' => 'change settings', 'class_list' => 'pink', 'action' => 'Item.AutofillSettingsModal'], ['text' => 'reset', 'class_list' => 'pink70', 'action' => 'Item.AutofillSettingsReset'], ['text' => 'close', 'class_list' => 'cancel']];
...
<div id='FormBuilder'>
    ...
    <div id="AddItem" class='prompt'>
        <div class='message'>
            ...
            <div id="FollowUpOptions" class='itemOptionList followupOptionList' data-type='followup'>
                ...
                <div class='settingsInfo parentInfo pink'>This question will only be asked when its parent question has
                    a specific response.</div>
            ...
            <div id="AutofillOptions" class='itemOptionList' data-type='autofill'>
                ...
                <div class="ButtonBox">
                    <div id='AutofillSettingsBtn' class="button pink xsmall" data-action="Item.AutofillSettingsPrompt">
                        autofill restrictions</div>
            ...
            <div id='Options' class='itemOptionList' data-type='["list","checkboxes","dropdown"]'>
                ...
                <div class='settingsInfo'>add as many as you'd like. use enter key to move down. use arrows to rearrange
                </div>
                <div id='OptionsList' class='optionsList'>
                    ...
                    <div class="button_box">
                        ...
                        <div class='button small yellow' data-action='Item.ShowAutofillList'>autofill with category
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id='TextOptions' class='itemOptionList' data-type='["text","phone"]'>
                ...
                <div class='button small yellow NewLinkBtn' data-action='Item.ShowAutofillList'>add category popup
                </div>
            </div>
            ...
            <div id='TextBoxOptions' class='itemOptionList' data-type='textbox'>
                ...
                <div class='button small yellow NewLinkBtn' data-action='Item.ShowAutofillList'>add category popup</div>
            </div>
            ...
            <div id='ScaleOptions' class='itemOptionList' data-type='scale'>
                ...
                <div class='settingsInfo'>Labels will always show on each side, but you can choose to show the values or
                    not.<br>To approximate a 'visual analog scale', hide the values and set the range to '0 to 100'.
                </div>
            ...
            <div id='DateOptions' class='itemOptionList' data-type='date'>
                ...
                <div class='settingsInfo'>Which Dates should be Available?<br><span>(ex: 1 week before/after current
                        date)</span>
                </div>
    ...
    <div id="LinkedItem" class="modalForm">
        <div id="AutofillList" class='modalForm center max'>
            ...
            <div class="List" data-header='Available Categories' data-action="Item.SelectAutofillModel"
                data-json='{{ json_encode($categories) }}' data-ul_class='horizontal center box'></div>
        </div>
        <div id="AutofillSettingsPrompt" class='OptionBox' data-header="Current Autofill Settings"
            data-message="No restrictions. All available items will be loaded."
            data-buttons="{{ json_encode($AutofillSettingsPrompt_buttons) }}"
            data-key_value_options="{{ json_encode($AutofillSettingsPrompt_kv_options) }}"></div>
    </div>
</div>
