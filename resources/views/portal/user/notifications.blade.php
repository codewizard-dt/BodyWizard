...
<ul>
    @forelse($notifications as $notification)
        ...
        ?>
        <li data-notificationid='{{$notification->id}}' data-model='{{$model}}' data-uid='{{$uid}}' data-changes='{{$changes}}' data-details='{{$details}}' data-tabid='{{$tabId}}' data-type='{{$type}}' data-click='{{$click}}' data-description='{{$description}}'>{{$type}}<span class="indicator {{$indicatorStatus}}"></span></li>
    @empty
