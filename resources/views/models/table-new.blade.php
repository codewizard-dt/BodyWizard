...
<div class='wrapMe marginBig bottomOnly' style='display:inline-block'>
	<div id="TableButtons" data-type="{{$tableType}}">
    	...
    	<div class='button xsmall createNew pink70' data-model='{{$nospaces}}' data-mode='modal' data-target='CreateNew{{$nospaces}}' data-action='Answer.reset_all'>Add New {{$displayName}}</div>
	...
	<div>
		<table id='{{$nospaces}}List' class='styledTable clickable modelTable' data-index="{{$index}}" data-display='{{$displayName}}' data-target="#Current{{$nospaces}}" data-model='{{$nospaces}}' data-hideorder='{{json_encode(camel($hideOrder))}}' data-significance='{{$tableType}}' data-namecolumn='{{$nameColumn}}'>
